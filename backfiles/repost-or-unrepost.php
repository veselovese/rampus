...
if (isset($_POST['reposted'])) {
    ...
    $sql_reposted = "SELECT user_id, for_friends, text, hashtag_id, repost_user_id, repost_post_id, reposts FROM posts WHERE id = $post_id LIMIT 1";
    $result_reposted = $connect->query($sql_reposted);
    if ($result_reposted->num_rows > 0) {
        ...
        $sql_reposted_images = "SELECT image_url FROM images_in_posts WHERE post_id = $post_id ORDER BY add_date ASC";
        $result_reposted_images = $connect->query($sql_reposted_images);
            ...
            if ($repost_repost_user_id) {
                ...
                if ($connect->query("SELECT id FROM reposts WHERE post_id = $repost_repost_post_id AND user_id = $user_id")->num_rows == 0) {
                    ...
                    if ($connect->query($sql_repost_post_id)->num_rows == 1) {
                        ...
                        $connect->query("INSERT INTO posts (user_id, hashtag_id, text, repost_post_id, repost_user_id) VALUES ('$user_id', $hashtag_id_value, '$repost_text', '$repost_repost_post_id', '$repost_repost_user_id')");
                        $current_post_id = $connect->query("SELECT @@IDENTITY AS id")->fetch_assoc()['id'];

                        if ($result_reposted_images->num_rows > 0) {
                            while ($row_reposted_images = $result_reposted_images->fetch_assoc()) {
                                $image_url = $row_reposted_images['image_url'];
                                $connect->query("INSERT INTO images_in_posts (post_id, image_url) VALUES ($current_post_id, '$image_url')");
                            }
                        }
                    }
                ...
                if ($connect->query("SELECT id FROM reposts WHERE post_id = $post_id AND user_id = $user_id")->num_rows == 0) {
                    ...
                    if ($connect->query($sql_repost_post_id)->num_rows == 1) {
                        ...
                        $connect->query("INSERT INTO posts (user_id, hashtag_id, text, repost_post_id, repost_user_id) VALUES ('$user_id', $hashtag_id_value, '$repost_text', '$post_id', '$repost_user_id')");
                        $current_post_id = $connect->query("SELECT @@IDENTITY AS id")->fetch_assoc()['id'];
