...
if ($the_likest_post->num_rows > 0) {
    ...
    $trophy_the_likest_post = $connect->query("SELECT user_id_to, user_id_from FROM trophies WHERE (NOT user_id_to = $user_id_to OR NOT stat_number = $the_likest_post_likes) AND id = 4");
...
if ($users->num_rows > 0) {
    ...
    $trophy_likes_on_one_post = $connect->query("SELECT user_id_to, user_id_from FROM trophies WHERE (NOT user_id_to = $user_id_to OR NOT stat_number = $max_likes_on_posts) AND id = 5");
...
if ($users->num_rows > 0) {
    ...
    $trophy_likes_to_other_users = $connect->query("SELECT user_id_to, user_id_from FROM trophies WHERE (NOT user_id_to = $user_id_to OR NOT stat_number = $max_likes_to_other_users) AND id = 6");
...
$max_comments_under_one_post = 0;

if ($posts->num_rows > 0) {
    while ($row = $posts->fetch_assoc()) {
        $current_post_id = $row['id'];
        $the_commentest_post = $connect->query("SELECT user_id FROM comments WHERE post_id = $current_post_id");
        $comment_counter = $the_commentest_post->num_rows;
        if ($comment_counter > $max_comments_under_one_post) {
            $row = $the_commentest_post->fetch_assoc();
            $user_id_to = $row['user_id'];
            $max_comments_under_one_post = $comment_counter;
            $post_id = $current_post_id;
        }
    }
    $trophy_comments_under_one_post = $connect->query("SELECT user_id_to, user_id_from FROM trophies WHERE (NOT user_id_to = $user_id_to OR NOT stat_number = $max_comments_under_one_post) AND id = 7");
...
if ($users->num_rows > 0) {
    ...
    $trophy_comments_on_posts = $connect->query("SELECT user_id_to, user_id_from FROM trophies WHERE (NOT user_id_to = $user_id_to OR NOT stat_number = $max_comments_on_posts) AND id = 8");
...
if ($users->num_rows > 0) {
    ...
    $trophy_comments_on_posts = $connect->query("SELECT user_id_to, user_id_from FROM trophies WHERE (NOT user_id_to = $user_id_to OR NOT stat_number = $max_comments_to_other_posts) AND id = 9");
    if ($trophy_comments_on_posts->num_rows == 1) {
        $row = $trophy_comments_on_posts->fetch_assoc();
        $check_user_id_to = $row['user_id_to'];
        $check_user_id_from = $row['user_id_from'];
...
if ($the_repostest_post->num_rows > 0) {
    ...
    $trophy_the_repostest_post = $connect->query("SELECT user_id_to, user_id_from FROM trophies WHERE (NOT user_id_to = $user_id_to OR NOT stat_number = $the_repostest_post_reposts) AND id = 10");
...
if ($users->num_rows > 0) {
    ...
    $trophy_reposts_on_posts = $connect->query("SELECT user_id_to, user_id_from FROM trophies WHERE (NOT user_id_to = $user_id_to OR NOT stat_number = $max_reposts_on_posts) AND id = 11");
...
if ($users->num_rows > 0) {
    ...
    $trophy_reposts_to_other_posts = $connect->query("SELECT user_id_to, user_id_from FROM trophies WHERE (NOT user_id_to = $user_id_to OR NOT stat_number = $max_reposts_to_other_posts) AND id = 12");
...
if ($users->num_rows > 0) {
    ...
    $trophy_posts_from_one_user = $connect->query("SELECT user_id_to, user_id_from FROM trophies WHERE (NOT user_id_to = $user_id_to OR NOT stat_number = $max_posts_from_one_user) AND id = 13");
...
if ($users->num_rows > 0) {
    ...
    $trophy_friends_in_one_user = $connect->query("SELECT user_id_to, user_id_from FROM trophies WHERE (NOT user_id_to = $user_id_to OR NOT stat_number = $max_friends_in_one_user) AND id = 14");
