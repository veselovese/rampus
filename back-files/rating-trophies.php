...
if ($the_likest_post->num_rows > 0) {
    ...
    if ($trophy_the_likest_post->num_rows == 1) {
        ...
        $user_id_from = $check_user_id_to != $user_id_to ? $check_user_id_to : $check_user_id_from;
...
if ($users->num_rows > 0) {
    ...
    $trophy_comments_on_posts = $connect->query("SELECT user_id_to, user_id_from FROM trophies WHERE (NOT user_id_to = $user_id_to OR NOT stat_number = $max_comments_on_posts) AND id = 9");
    if ($trophy_comments_on_posts->num_rows == 1) {
        $row = $trophy_comments_on_posts->fetch_assoc();
        $check_user_id_to = $row['user_id_to'];
        $check_user_id_from = $row['user_id_from'];
        $user_id_from = $check_user_id_to != $user_id_to ? $check_user_id_to : $check_user_id_from;
        $connect->query("UPDATE trophies SET user_id_to = $user_id_to, user_id_from = $user_id_from, stat_number = $max_comments_on_posts, get_date = NOW() WHERE id = 9");
    }
}
...
if ($users->num_rows > 0) {
    ...
    $trophy_posts_from_one_user = $connect->query("SELECT user_id_to, user_id_from FROM trophies WHERE (NOT user_id_to = $user_id_to OR NOT stat_number = $max_posts_from_one_user) AND id = 10");
    if ($trophy_posts_from_one_user->num_rows == 1) {
        $row = $trophy_posts_from_one_user->fetch_assoc();
        $check_user_id_to = $row['user_id_to'];
        $check_user_id_from = $row['user_id_from'];
        $user_id_from = $check_user_id_to != $user_id_to ? $check_user_id_to : $check_user_id_from;
        $connect->query("UPDATE trophies SET user_id_to = $user_id_to, user_id_from = $user_id_from, stat_number = $max_posts_from_one_user, get_date = NOW() WHERE id = 10");
    }
}
...
if ($users->num_rows > 0) {
    ...
    $trophy_friends_in_one_user = $connect->query("SELECT user_id_to, user_id_from FROM trophies WHERE (NOT user_id_to = $user_id_to OR NOT stat_number = $max_friends_in_one_user) AND id = 11");
    if ($trophy_friends_in_one_user->num_rows == 1) {
        $row = $trophy_friends_in_one_user->fetch_assoc();
        $check_user_id_to = $row['user_id_to'];
        $check_user_id_from = $row['user_id_from'];
        $user_id_from = $check_user_id_to != $user_id_to ? $check_user_id_to : $check_user_id_from;
        $connect->query("UPDATE trophies SET user_id_to = $user_id_to, user_id_from = $user_id_from, stat_number = $max_friends_in_one_user, get_date = NOW() WHERE id = 11");
    }
}
