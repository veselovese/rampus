...
$ogTitle = "Чат с пользователем в Рампусе";
$ogDesc = "Перейдите, чтобы написать";
$ogImage = $baseUrl . "/pics/plugs/RampusSmallPlug.png";
$ogUrl = $baseUrl;

$result_other_user = $connect->query("SELECT id, username, first_name, second_name, avatar, verify_status FROM users WHERE username = '$other_user_username' LIMIT 1");
if ($result_other_user->num_rows > 0) {
    ...
    $ogUrl = $baseUrl . "/chat/" . $other_user_username;
} else {
...
<?php
if (!isset($_SESSION['user'])) {
    header("Location: auth?r=chat&id=$other_user_username");
    exit();
}
?>
